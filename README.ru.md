...
- `tandem_work` по умолчанию возвращает `view="summary"`; `plan`, `step` и `full` запрашиваются явно. История разбита на страницы; используйте курсор, а при `cursor_stale` перечитайте состояние. `next_actions` — подсказки, не разрешения.
- Каждый результат содержит `runtime_identity`: сравнивайте загруженный пакет, сборку и схемы с рабочей копией. Обновление файлов не обновляет уже работающий сервер.
...
- `wake_acknowledgment` различает `acknowledged`, `deferred`, `not_attempted`. Приёмка не означает применение: без свидетельств — `not_recorded`; `assess`, квитанции apply и публикация учитываются отдельно. Native-расходы — ограниченный подытог; стоимость Claude без отдельных измерений неизвестна.

[Контракты, отчёты и восстановление](docs/guide.ru.md#compact-contracts). Версия пакета 3.6.0 предложена и ожидает подтверждения выпуска оператором; тег и публикация не подразумеваются.
