...
Текущая версия пакета — **3.10.0**. Одно длинное ограниченное ожидание заменяет вереницу коротких, отказ при захвате ревью называет все исправимые пути сразу, а закреплённый набор находок отвечает, что стало с каждой находкой прошлого круга. Всё — только добавления: значения по умолчанию, права и проверки приёмки не изменились. См. [CHANGELOG](CHANGELOG.md). Сборка/проверка пакета не создаёт тег, не публикует, не устанавливает пакет в пользовательскую сессию и не применяет код.
