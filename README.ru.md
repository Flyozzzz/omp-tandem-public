...
Текущая версия пакета — **3.8.1**. [Капсулы контекста, preflight, новые беседы и корректирующее ревью](docs/guide.ru.md#context-efficient-work) сокращают повторную координацию без снятия проверок прав и приёмки. См. [CHANGELOG](CHANGELOG.md). Сборка/проверка пакета не создаёт тег, не публикует, не устанавливает пакет в пользовательскую сессию и не применяет код.
