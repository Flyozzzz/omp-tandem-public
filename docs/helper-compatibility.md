...
unsatisfied gates; it is `available: false` in every run of this revision.
...
| `parent_abort_prevents_late_child_delivery` | supported | parent `abort` while the child stream was held: after releasing the held response no further model request arrived; two script steps stayed unconsumed |
...
| §3.2 cancel/deadline/owner-close, late results, revival | monotonic deadline + `client.stop()` (`native_worker.py:175-190`); `TaskRuntime.shutdown` owns its threads (`task_runtime.py:260-285`) | parent `abort` stopped the held child stream; no late request after release; `task.maxRuntimeMs`, `task.agentIdleTtlMs` (parked agents revive when messaged) exist | T for process stop; U/D for a tree-stop acknowledgement and unrevivable retirement | `parent_abort_prevents_late_child_delivery` |
...
| §5.5 whole-tree stop | process-group stop; owner fences | abort stopped the child stream in this run; no separate tree acknowledgement API | T for the observed stop; **U** for acknowledged stop and durable "stopped" state | `parent_abort_prevents_late_child_delivery` |
...
4. Cost accounting for helper trees cannot rely on the parent `TurnUsage`; a child
   ...
   prerequisites for §8.
5. Abort propagation and `task.disabledAgents` work as observed and can be
   relied upon once the above blockers are addressed.
