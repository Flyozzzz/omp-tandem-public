...
| `parent_abort_prevents_late_child_delivery` | supported | parent `abort` while the child stream was held; **observed before any process stop**, with the parent RPC session still answering: releasing the held child response produced no further model request (52 → 52) and the child-answer and parent-answer steps stayed unconsumed. The gate predicate requires that positive liveness observation (`parent_alive` from a successful `get_subagents`); a parent that no longer answers fails the gate even when no late request appears, because silence from a dead process is not abort semantics. This proves abort semantics, not process-group cleanup, which the SDK `stop()` provides separately |
