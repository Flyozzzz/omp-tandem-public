...
**Пакет 3.7.1 (локальное предложение; не опубликовано)** · [Лицензия MIT](../LICENSE) · [Релизы](https://github.com/Flyozzzz/omp-tandem-public/releases) · [Channels и вебхуки (на английском)](channels.md) · [Oh My Pi](https://github.com/can1357/oh-my-pi)
...
3.7.1 — patch-версия, **локальное предложение, не опубликовано**: операторские удобства, честное представление и регрессии поверх выпущенной 3.7.0. `uv build --wheel`, `uv run --frozen python scripts/package.py` и `package.py --check` готовят/проверяют локальные артефакты. Это не тег, публикация, установка в пользовательскую сессию или применение результата; решает оператор. [Исходные запросы](spec/plan-transition-and-git-root-2026-09-12.md) сохранены с provenance.
