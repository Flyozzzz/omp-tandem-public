...
**Пакет 3.7.0 (выпущен 2026-09-13)** · [Лицензия MIT](../LICENSE) · [Релизы](https://github.com/Flyozzzz/omp-tandem-public/releases) · [Channels и вебхуки (на английском)](channels.md) · [Oh My Pi](https://github.com/can1357/oh-my-pi)
...
Если MCP использует нестандартный `--state-dir`, передавайте тот же путь **перед подкомандой**. `authorize --preview` проверяет параметры и выводит `stored=false`, не сохраняя разрешение и не доказывая готовность задачи. Обычный `authorize` сразу активирует разрешение, закрепляя план, исходный коммит, модели, срок, число запусков и бюджет. `--allow-work` разрешает реализацию; **`--allow-shell` даёт произвольный shell, не только безопасные тесты**, а `--allow-tests` — устаревший синоним с теми же правами и предупреждением.

`--claude-model` (по умолчанию `sonnet`) и `--omp-model` (синоним `--model`) ставятся перед `authorize`. `run`/`start` не могут заменить модель действующего разрешения. Выбор и его происхождение не равны наблюдаемой фактической модели; настройки чужой интерактивной сессии не заимствуются.
