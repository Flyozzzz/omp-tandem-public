...
**Версия: 3.1.0** · [Лицензия MIT](../LICENSE) · [Релизы](https://github.com/Flyozzzz/omp-tandem-public/releases) · [Channels и вебхуки (на английском)](channels.md) · [Oh My Pi](https://github.com/can1357/oh-my-pi)
...
Для содержательного анализа и ревью такой двухэтапный порядок уменьшает привязку к постановке координатора. Используйте `tandem_start` для первой оценки и `tandem_continue`, чтобы раскрыть и сравнить предложение. Сохраняйте подтверждённые пользователем факты. Если решение уже видно в коде, истории или общем контексте, признайте это влияние, а не называйте ревью слепым. Простое исполнение с установленной целью не требует двух этапов.
...
`tandem_review(action="create")` сохраняет требования, критерии, выбранные байты рабочего дерева (`selected`), версии из базы (`base`) и индекса (`staged`), хеши и полученный из них diff. Он возвращает неизменяемый `review_id` и метаданные, включая `code_fingerprint`. Команды тестов **не выполняются автоматически**.
...
Без `paths` выбираются текущие неигнорируемые Git-изменения. Вне Git нужны явные относительные пути; такой снимок тоже поддерживается, но Git-версия неизвестна. Выбранные submodule и неслитые записи индекса отклоняются явно. Лимиты: 256 файлов, 4 MiB на файл, 16 MiB сохранённых материалов. Обнаружение изменений между чтениями ограничено и не превращает захват в транзакцию файловой системы.
