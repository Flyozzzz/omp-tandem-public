...
**Пакет 3.11.0** · [Лицензия MIT](../LICENSE) · [Релизы](https://github.com/Flyozzzz/omp-tandem-public/releases) · [Channels и вебхуки (на английском)](channels.md) · [Oh My Pi](https://github.com/can1357/oh-my-pi)
...
- [Контекст без повторов, preflight и новые беседы](#context-efficient-work)
...
### Выбор входа и граница проекта

Для ревью подготовленного коммита используйте `tandem_review_run` со снимком staged/индекса: без правок и shell. Для разработки двумя агентами — `tandem_work`, общий план, владельцы файлов и отдельный проверяющий. [Карта совместимости помощников](helper-compatibility.md) фиксирует `delegation.available=false` и пять непройденных условий: наследование ограничений инструментов, подмена scout проектом, снимок настроек всей задачи, отдельный учёт родителя и лишние модельные вызовы при spawn. Этапы C–F и экономия от помощников не выпущены.

Для claim/submit **сам закреплённый корень проекта должен быть Git-репозиторием с коммитом HEAD**. Сессия из родительской папки может получить `Work execution requires a Git repository with an immutable HEAD commit`. Откройте нужный репозиторий или исправьте привязку клиента; `cwd` задачи не меняет пространство проекта. Новому репозиторию нужен начальный коммит до исполнения.
...
Обе стороны используют **один `tandem_work(request: WorkCommand, wait_seconds=0..1200)`**. Место участника у обычного клиентского сервера по умолчанию называется `claude`, даже если основной клиент — Codex. Для второго доверенного клиента оператор задаёт `--work-participant omp` в его конфигурации запуска. Native OMP получает место `omp` от моста. Это роли внутри задачи, а не проверка личности модели: модель не передаёт `actor` и не меняет роль текстом запроса. Участники должны использовать один канонический `--project-root` и одну базовую директорию состояния; не привязывайте второго клиента к пространству имён отдельного worktree.

<a id="compact-contracts"></a>
### Компактные контракты, идентичность и закрытие

Параметры представления находятся рядом с `request`: `view="summary"|"plan"|"step"|"full"`, `format="json"|"markdown"`, `limit`, `cursor`, `include_snapshots`. Для выбранного шага задайте `request.step_id`. По умолчанию возвращается summary без дублирования Markdown; история разбита на страницы без снимков. Следуйте продолжению: курсор связан с карточкой, ревизией, разделом и видимостью; `cursor_stale` требует нового чтения. `next_actions` показывает условия и идентификаторы, но не выдаёт разрешение и не означает автоматический повтор. CAS и смысл `operation_id` не меняются.

Каждый результат содержит `runtime_identity`: версия загруженного пакета, источник установки, сборка по проверенному `RECORD`, наблюдение рабочей копии и хеши зарегистрированных схем. Для editable/непроверяемой сборки остаётся `unknown`; новые файлы не обновляют работающий сервер. Запрошенная, эффективная и наблюдаемая модели, ручной/управляемый режим, grant и раскрытие учитываются раздельно, без скрытого модельного запроса.

Независимое ревью: `report` → не более одного `compare` → точный `accept`/`reject`; успешная доставка не является вердиктом. До сравнения уточнение возвращает `clarification_requires_new_snapshot` и закрывает затронутую стадию, не передавая текст автора. Добавьте отсутствующие неизменённые файлы точными относительными `review_context_paths` в новый согласованный снимок: без glob, обхода путей, разыменования symlink и чтения живой копии. Исключение связано с планом, сдачей, grant и входами снимка; история сохраняется, но не разрешает новые входы. Ручной протокол не создаёт ОС-песочницу.

Дескриптор `recovery` показывает захват, стадию, сдачу, сохранённые материалы, действия и условия остановки/reconcile, но не секрет. `recovery_not_authorized` и другие коды — условия, не команда повторять. После подтверждённой остановки оператор через подготовленный Python явно разрешает одного преемника:

```sh
python -m omp_tandem.work_daemon --project-root /project --state-dir /state successor ATTEMPT_ID --host HOST_OWNER --principal claude --note 'Закрытие отчётом по сохранённым свидетельствам'
```

Этот хост делает `recover` со свежей ревизией и operation ID, затем отчёт и точный вердикт по тому же действующему захвату. Модель/тесты не перезапускаются; сдача, стадия, срок и grants не меняются. Чужой, истёкший, отозванный, legacy-unbound или неподтверждённо остановленный захват не получает новых полномочий. Старые квитанции читаются как история. Суффикс `(code=cyber_policy)` классифицируется как `provider_policy_refusal`; слово в обычном тексте — нет. Отказы стадии ревью представлены кодами причин; административное закрытие не обходит отказ провайдера.

`wake_acknowledgment`: `acknowledged`, `acknowledged_zero` (нулевой счётчик), `deferred` с причиной или `not_attempted`, для наблюдённых карточки/ревизии. Отложенное подтверждение не отменяет прочитанное состояние; следующее явное чтение может подтвердить сохранённые подсказки. Поздние/повторные уведомления не запускают работу и не подтверждают будущие ревизии.

Приёмка, применение и публикация раздельны. Без свидетельств применения — `not_recorded`. `assess` фиксирует ожидаемый/наблюдаемый HEAD и время, но не автора/метод; явный `apply --expected-head` оставляет отдельную квитанцию. `show WORK_ID --format markdown` выводит отчёт; `format="markdown"` для действий инструмента кроме get — JSON в ограждённом блоке. Подтверждённо связанные native-ходы считаются один раз, частичная стоимость — подытог; Claude и несвязанные расходы остаются неизвестными. Меньший JSON не означает пропорционального ускорения или экономии.

Исходный сбой, применимый последующий успех, доставка и приёмка не сливаются. Свидетельство равного immutable-tree применимо только при совпадении остальных входов проверки; команды, зависящие от метаданных, и новый scope не эквивалентны. Один сбой очистки `TemporaryDirectory` в `tests/test_review_runs.py` исчез при повторах, но причина не установлена. Проверка схем охватывает только исполненный `openai-completions`, не все API; помощники выключены.
...
Проверяющий `omp` захватывает ревью и читает **именно сохранённый коммит**, не меняющееся рабочее дерево. Сначала обязателен независимый `report`; затем возможно одно сравнение `compare`, и только потом отдельное решение:
...
# Перед каждым вызовом перечитать R; report содержит полный независимый отчёт.
tandem_work(
    request={
        "action": "report",
        "work_id": WORK_ID,
        "step_id": "parser",
        "expected_revision": R,
        "operation_id": "parser-independent-report-1",
...
# Необязательно, только после успешного полного отчёта, не более одного раза:
tandem_work(
    request={
        "action": "compare",
        "work_id": WORK_ID,
...
Новые попытки используют `independent_first`. Полный отчёт хранится неизменяемо с `resolution="success"`, `"partial"` или `"blocked"`; успех означает завершённую оценку, не отсутствие дефектов. Приёмка и сравнение доступны только после успешного отчёта. Отклонить или заблокировать результат можно по независимым доказательствам без сравнения. Аргументы автора, свободный текст evidence и связанные артефакты остаются скрытыми **до открытия compare**, даже после отчёта.

Управляемые Claude/OMP-проверяющие по-прежнему читают закреплённый коммит через серверный reader, без живых файловых инструментов и произвольного shell. Старые shell-гранты сохраняют операторский блокер; участник не может снять его или спрятать обязательную команду в тексте. Новый явный grant ниже разрешает исполнение коду супервайзера, а не модели-ревьюеру; это не песочница ОС.

<a id="controlled-review-checks"></a>
#### Проверки внутри продуктового шага

В `review_verification` шага задаются реальные команды и выбранная накопительная стадия. После успешного неизменяемого независимого отчёта супервайзер выполняет каждую команду в новом Linux Docker-контейнере, подключая только отдельную точную копию предъявленного коммита. Общей Git-метаинформации с автором или reviewer нет. Второй продуктовый шаг только ради прогона не нужен. Ручные shell-claims не расширяются.
...
Preview не выдаёт прав: обычный authorize требует явного решения оператора. Нужны работающий локальный Linux Docker engine, Docker CLI и заранее подготовленный образ с `/bin/sh` и инструментами проекта; для Bun-проекта — соответствующий Bun-образ. Образ автоматически не скачивается, выполнения на хосте вместо контейнера нет. Текущий Docker context или `--review-check-docker-context NAME` разрешается в закреплённый локальный socket/daemon; образ закрепляется по неизменяемому ID. Смена тега не меняет грант.

Сеть по умолчанию отключена. `--review-check-network NAME` явно разрешает существующую bridge-сеть и закрепляет её ID. `localhost` внутри контейнера — не хост; адрес тестовой БД и `--review-check-env TEST_DATABASE_URL` настраиваются явно. Сохраняются имена и хэши выбранных значений; отсутствующее или изменившееся значение запрещает запуск. Используются PATH образа и приватные HOME/TMPDIR; остальные ENV образа обнуляются, если не выбраны явно. Окружение модели не наследуется, исполнитель сам не загружает `.env`, но проектные команды могут это делать. Не засчитывайте cache hit как новый прогон, если нужен свежий запуск.

Rootfs доступна только для чтения, PID/IPC и `/tmp` приватны, capabilities сняты, действует no-new-privileges. Docker socket и исходный репозиторий не монтируются. Подтверждение остановки требует наблюдаемого удаления контейнера на том же daemon: завершение shell, EOF и `ps` недостаточны. Потерянный ответ create/start не разрешает повторный запуск; неизвестная остановка требует reconcile. Контейнер не откатывает изменения БД/API и не обещает универсальную защиту от враждебного кода; внешние расходы команд не входят в стоимость моделей. Игнорируемые невладельческие зависимости не закрепляются как исходники; их symlink/hardlink не ломают сдачу, но tracked/owned-входы остаются строгими.

Пока проверки не завершены, используйте обычный bounded get. До compare раздел `verification` показывает только статус; затем доступны ограниченные логи по непрозрачным `verification/<run-id>` и `next_cursor`. Accept требует compare и всех выбранных текущих проверок со статусом passed; ошибку можно прочитать и отклонить результат, а не объявить успехом тестов. Записи machine_observed создаёт только код. Отмена, изменение входов и неизвестное исполнение не дают автоматического повтора.

Управляемый submit проверяет владение до записи намерения. `intent_recorded` при `output_committed=false` — ещё не коммит; только успешный финальный capture создаёт submission. Ошибка `capture_failed` требует восстановления с сохранением полезных правок, не удаления нужных тестов. Старые гранты и запущенные попытки новых прав не получают. [Полный контракт](guide.md#controlled-review-checks).
...
5. **Продолжение результатов, не команд:** `checkpoint` означает, что проверенные правки входят в новое владение; `not_transferable` — шаг удалён или владение исключает сохранённые файлы; `not_available` — проверенных байтов нет, включая capture failure. Это возможность переноса, **не автоматические блокеры и не готовность к запуску**. Старые записи 3.7.0 `blocked` отображаются как `not_transferable` без переписывания истории. Checkpoint хранит commit/base и свидетельства оператора, передаётся новому claim, но не является submission или перенесённой приёмкой. Старые команды автоматически не воспроизводятся.

До активации осмотрите `activation_preview` в get, `transition inspect` или Markdown: `steps_with_checkpoint`, `steps_undetermined` (disposition ещё не определён), `steps_without_checkpoint`, `capture_failures`, `capture_failures_unacknowledged`. По каждой попытке видны сохранённый commit/файлы и исход переноса. `ready` означает только завершённый разбор инвентаря, **не готовность запуска**: пауза, блокеры, согласования и grant проверяются отдельно.

Каждой superseded-попытке с `saved.capture_failure` нужен осознанный операторский `--acknowledge-capture-failure ATTEMPT` при activate; повторите флаг для каждого точного ID. Без него — `capture_failure_unacknowledged`, с посторонним ID — `capture_failure_unknown`. Подтверждение входит в идентичность команды и исход: правки не удалось захватить, они потеряны. Успешное сохранение подтверждения не требует. `--abandon` одинаково записывает `abandoned` в попытку, инвентарь и квитанцию, не переносит continuation и оставляет блокер/паузу; это не `superseded`.
...
Сценарии используют настоящий Git, FastMCP и CLI во временных репозиториях. Managed-сценарий подставляет детерминированный исполняемый файл Claude в настоящий `WorkSupervisor`: привязанные get/heartbeat, одна запись внешнего эффекта и маркер собственного файла, реальная отмена/reap, подтверждение supervisor, неизменяемое сохранение, activate и явные resume/agree/authorize. Тот же процесс-программа продолжает checkpoint по маркеру; после двух claim запись эффекта ровно одна. Стоимость прерванной попытки остаётся неизвестной; grant на один запуск исключает ревью OMP. Это проверяет **границу Claude-child/shared-supervisor**, не общую exactly-once гарантию, не OMP-native teardown и не live-provider replan. [Совместимость](compatibility.md) — отдельный pinned OMP/localhost fixture.

3.7.1 — patch-версия (выпущена 2026-09-13): операторские удобства, честное представление и регрессии поверх выпущенной 3.7.0. `uv build --wheel`, `uv run --frozen python scripts/package.py` и `package.py --check` готовят/проверяют локальные артефакты. Это не тег, публикация, установка в пользовательскую сессию или применение результата; решает оператор. [Исходные запросы](spec/plan-transition-and-git-root-2026-09-12.md) сохранены с provenance.

### Карточка, блокеры и приватные права попытки

`get` по умолчанию возвращает JSON summary без дублирующего поля `markdown`. Представления `plan`, `step`, `full` и `format="markdown"` запрашиваются явно. Показывайте пользователю карточку и её продолжения, а не только «агент занят». `list` перечисляет задачи проекта; `history` сохраняет историю изменений и возвращает страницы. Проверяйте `status`, `plan_revision`, `agreements`, `steps`, `authorization` и актуальность `result`, прежде чем продолжать работу.
...
#### Снятие блокера оператором

Автор блокера использует MCP; доверенный оператор может выполнить тот же доказательный переход через CLI:
...
Обязательны `--blocker`, `--resolution`, `--note`, хотя бы один `--evidence`, актуальная ревизия карточки `--expected-revision` и новый `--operation-id`. `--evidence` повторяемый. `resolved` объясняет выполнение условия, `not_applicable` — почему оно больше не применимо. `--step` указывает **текущее** местоположение блокера; для уровня карточки, включая перенесённый при activate блокер, его опускают. `operator_commands`, `next_actions` и Markdown `## Operator commands` показывают текущую позицию, не origin. Подсказки остаются `allowed=false`, `operator_required`; агент не получает полномочий. Действуют CAS и точное воспроизведение операции. Unblock **не возобновляет паузу и не авторизует исполнение**: resume и действующий grant — отдельно.
...
```sh
python -m omp_tandem.work_daemon --project-root /absolute/project \
  --claude-model sonnet --omp-model <provider/model> authorize WORK_ID \
  --budget-seconds 1800 --max-launches 6 --max-cost-usd 5 \
  --max-attempt-cost-usd 2.5 --allow-work --preview
# После проверки preview и явного одобрения оператором повторите без --preview.
...
Если MCP использует нестандартный `--state-dir`, передавайте его перед подкомандой. `authorize --preview` проверяет параметры и выводит `stored=false`, не сохраняя grant и не доказывая готовность. После явного одобрения обычный `authorize` закрепляет план, исходный коммит, **политику выбора моделей**, срок, число запусков и бюджет. `--allow-work` разрешает реализацию; **`--allow-shell` даёт произвольный shell, не тестовую песочницу**, `--allow-tests` — устаревший синоним с теми же правами.

`preview.model_selection` раскрывает политику каждого участника. Claude — `fixed_selector`, по умолчанию `sonnet`; явный `--omp-model` (синоним `--model`) тоже `fixed_selector`, сохранённый при авторизации. Без него OMP — `dynamic_default`: настроенный выбор определяется **при старте каждой попытки**, конкретная модель не закреплена. Для предсказуемых автономных запусков рекомендуется `--omp-model <provider/model>`. Флаги моделей ставятся перед `authorize`; `run`/`start` не заменяют политику. `legacy_unpinned` требует новой авторизации; `malformed` помечается при чтении и запрещает запуск. Селектор не доказывает фактическую модель. Markdown `## Authorization` показывает сохранённое окно authorized_at/deadline, права и политику каждого участника, не удостоверяя отвечавшую модель.

`--max-attempt-cost-usd` по умолчанию равен половине общего бюджета независимо от `--max-launches`; явное значение ограничивается общим бюджетом. Попытка атомарно резервирует `min(предел попытки, общий бюджет − известные расходы − активные резервы)`; параллельные шаги делят остаток в порядке запуска, не обязательно поровну. Неизвестная стоимость останавливает новые запуски. В разрешении и `show` видны `max_attempt_cost_usd`, `attempt_cost_policy`, `preview.reserve_policy` и `preview.permissions`: `read`, `edit_write`, `shell`, неограниченный для процесса `network`, `os_sandbox=false`. Это не жёсткий предел счёта провайдера.
...
### Миграция и пределы отката

Старые разрешения сохраняют исторический предел попытки `max_cost_usd / max_launches` (`legacy_launch_share`) и декодирование `allow_tests`; права и активные резервы не расширяются. Старое ревью помечается `legacy_disclosure`, не признаётся независимым задним числом. Неоднозначный старый выбор OMP требует явного нового разрешения. История блокеров, расходы и recovery сохраняются; чтение и миграция не разрешают перезапуск. Неопределённое исполнение сначала сверяют и восстанавливают.

Импорт прежнего проекта — только копирование, источник не удаляется. Автоматический downgrade, откат внешних эффектов и безопасное повторение не гарантируются. Перед ручным backup/restore остановите все записывающие клиенты/контроллеры и сохраняйте полное состояние с удерживаемыми worktree по своей процедуре резервирования; копировать один работающий SQLite-файл недостаточно, а восстановление базы не отменяет внешние процессы.
...
## Контекст без повторов, preflight и новые беседы

### Объявить требования до запуска

У `tandem_start` и `tandem_continue` есть `preflight=true`: та же подготовка допуска без задачи, worker, модельного запроса и резервирования слота. Настоящий start проверяет условия повторно под блокировками. Preview не является разрешением или зарезервированным местом. Пример — замените учебные пути существующими файлами:
...
```json
{
  ...
  "timeout_seconds": 180,
  "contract": {
    "goal": "Проверить границу сервиса до изменения",
    "requirements": {
      "requires_shell": true,
    ...
    "verification": {
      ...
      "checks": [{
        "id": "service-boundary",
        "criterion": "Граница сервиса соответствует согласованным случаям",
        "phase": "targeted",
...
Пути должны принадлежать допустимому Git-root; границы вложенных репозиториев/worktree и симлинки не пересекаются молча. `requires_shell`/`requires_write` объявляют потребность, а не выдают право: analyze/think не получают shell от такого флага. Непустая команда проверки также означает потребность в shell. `live_path_evidence` содержит существующие artifact IDs и остаётся **атрибутированным свидетельством, не доказательством живого production-маршрута**. Необъявленные пути не угадываются из прозы.

Лестница накопительная: `candidate` включает targeted, `integration` — все три фазы. Подготовка и известные оценки не должны превышать общий deadline; неизвестное время остаётся неизвестным. Preflight не исполняет команды. Для success при явной лестнице нужны все выбранные `check_id`, соответствующие `run_id` и актуальные успешные записи `check_runs`; прежние неудачи остаются в истории. Успешный ход reviewer сам по себе не принимает submission.

Для известного текущего кандидата проверка может объявить `scope` существующего типа CheckScope: реальный `kind` (`tree`, `commit`, `archive` или `content`) и `digest`. Прогоны других байтов остаются в аудиторской истории, но не являются текущим доказательством. Без явной scope неоднозначные failures других входов остаются нерешёнными. Все текущие группы criterion/role должны пройти: один pass не скрывает failure другой роли. `result.verification` и `facts.checks` оценивают объявленные текущие входы, `check_runs` сохраняет всю историю. Допускается до 200 run-записей при максимум 50 объявленных проверках; превышение отклоняется, не обрезается.

У shared step поля `requirements`/`verification` относятся к реализации, `review_requirements`/`review_verification` — отдельно к проверяющему. Требование записи для reviewer недопустимо. Обязательный shell-check не обходит snapshot-only режим или отсутствующее разрешение: незакрытые проверки остаются blocked/partial, не снимаются молча.

### Объявленные приёмочные доказательства

Зелёный список проверок спокойно уживается с требованиями, которых не коснулся никто: проверки — это те, которые кто-то не забыл объявить, и они же служат себе знаменателем. `acceptance_set` делает знаменателем объявленные критерии.

Передайте в контракте тот же список приёмки в структурном виде — тексты обязаны повторять `acceptance` дословно и в том же порядке, чтобы не появилось второго источника истины, — и дайте каждому критерию устойчивый `id`. Критерий, состоящий из нескольких утверждений, объявляет их как `obligations`: у каждого свой `id`, свой текст и то `environment`, в котором это обязательство должно быть подтверждено:

```json
{"acceptance": ["Отмеченная задача в демо зачёркнута, встаёт на своё место по порядку вставки, счётчики пересчитываются, общий не меняется."],
 "acceptance_set": {"set_id": "<uuid>", "revision": "<sha256 от items>", "items": [
   {"id": "AC-FR01-04", "text": "Отмеченная задача в демо зачёркнута, встаёт на своё место по порядку вставки, счётчики пересчитываются, общий не меняется.",
    "obligations": [{"id": "strike", "text": "зачёркнута", "environment": {"browser": "webkit"}},
                    {"id": "position", "text": "встаёт на своё место по порядку вставки"},
                    {"id": "counters", "text": "счётчики пересчитываются"},
                    {"id": "total", "text": "общий счётчик не меняется"}]}]}}
```

Этот критерий — **четыре** единицы доказательства, а не одна. `VerificationCheck` через `acceptance_refs` называет, что именно он собирается проверить, связь многие-ко-многим, а ссылка на родительский критерий не засчитывает ни одного из объявленных обязательств: иначе одна проверка против критерия из четырёх утверждений читалась бы как покрытие.

Ревизия — это содержимое набора, а не номер версии, который вы выбираете: правка текста критерия меняет её, а доказательства, записанные против прежней ревизии, остаются видимыми как история, но не как текущий зачёт. У проверки та же идея: `check_revision` привязывает прогон к тому объявлению, которое он наблюдал, поэтому смена команды или сопоставления не сохраняет прежний зачёт.

`result.acceptance_coverage` даёт по строке на единицу и держит порознь то, что действительно различается: единица, которую никто не сопоставил; сопоставленная проверка, которую лестница не выбрала; выбранная проверка вообще без прогона; прогон, прямо сказавший `not_run`; текущая неудача; успех против других байтов; прогон в окружении, отличном от требуемого; прогон, окружение которого не записано вовсе (неизвестно — это не совпадение); устаревшая ревизия проверки; прогон сопоставленной проверки, не записавший сопоставления. Эти случаи не исключают друг друга: у одной единицы действительно могут быть разом объявление, текущая неудача и более старый успешный прогон.

`acceptance_coverage: "require_current_evidence"` в плане превращает эту картину в допуск: отчёт об успехе отклоняется, пока хоть у одной объявленной единицы нет применимого текущего успешного прогона. По умолчанию остаётся `report_only`, который не отклоняет ничего. Строгий запрос без объявленного набора отклоняется, а не понижается молча; пустой набор сообщает `not_assessable`, а не пустопорожнюю пригодность.

Задача, не объявившая набора, сообщает `assessment: "not_declared"` — с числом строк приёмки для ориентира и без знаменателя. Это не нулевое покрытие: выдумать идентичности для обычных строк значило бы обвинить каждого существующего вызывающего в сопоставлениях, которых он не делал.

**Чего это никогда не утверждает.** Проекция считает объявления и прогоны. Она не устанавливает, что сопоставленная проверка утверждает правильное поведение, что объявленные обязательства исчерпывают критерий и что успешный прогон означает выполненное требование: `semantic_sufficiency` и `acceptance` остаются `not_assessed` в любом ответе, в том числе когда у каждой единицы есть текущий успех. Участник может объявить сопоставление, которого его тест не соблюдает, и ни одна схема этого не обнаружит; проекция делает утверждение явным и пригодным для разбора — это другое, чем его удостоверить.

Объявленное покрытие `acceptance_set` остаётся локальным для задачи. Шаги общей работы отклоняют такие сопоставления через `acceptance_coverage_unsupported_surface`; связанная с ними задача сообщает `assessment: "unsupported_surface"`, не применяя свой набор к чужому плану. Прогоны супервайзера фиксируют исполнение объявленных проверок, но не добавляют сопоставления acceptance-set в карточки и не доказывают смысловую достаточность тестов.

### Закреплённые капсулы вместо полного snapshot каждый ход

`contract.context_options.delivery` по умолчанию `capsule`; `full` явно включает все данные выбранного снимка. Капсула сохраняет полный текст/источник/применимость required rules и текущие accepted/rejected/deferred решения с происхождением. Тела advisory rules и superseded решений можно выбрать через `advisory_rule_ids`/`decision_ids`; неизвестные IDs отклоняются до запуска.

Первый, изменённый и fresh-контекст содержит обзор продукта. Продолжение с тем же snapshot передаёт immutable ID/SHA и обязательные инварианты без повторения обзора и примеров. Это **не предположение, что native compaction сохранила старые сообщения**. Оставшиеся данные читаются через `tandem_context_read` по возвращённым RFC 6901 `pointer`, `offset_bytes` и `next_offset_bytes`. Страница — 4–16384 байта UTF-8; соедините `content` и разберите канонический JSON. Reader читает только снимок текущей задачи, не произвольную последнюю ревизию или файл. В MCP-версии координатора дополнительно передаётся `task_id`. Независимое ревью использует свой объявленный review-reader.

Managed-попытка получает свой шаг, полные обязательные глобальные ограничения/критерии, точные зависимости и краткий обзор со ссылками на полный план. Тела остальных шагов и авторские отчёты не дублируются. Снижение размера контекста не означает такую же долю экономии стоимости провайдера.

Не совмещайте independent `review_id` с отдельным `project_context_id`: этот дополнительный канал запрещён и при допуске, и при сборке сообщения. Нужные требования/policy/context включаются в review bundle. Comparison требует полного успешного структурированного отчёта именно этого снимка без требования recapture; условие повторно проверяется под conversation lease и в транзакции вставки.

### Явно начать новую беседу

Обычный `tandem_continue` сохраняет `continuation="resume"`. Для нового самостоятельного результата используйте ограниченный handoff с источниками:
...
```json
{
  ...
  "continuation": "fresh",
  "handoff": {
    "reason": "Новый ограниченный результат",
    "summary": "Предыдущая граница проверена на сохранённом коммите.",
    "remaining_goals": ["Проверить следующий вход сервиса"],
    "invalidated_assumptions": ["Старый FSM не является живым маршрутом"],
    "evidence_artifact_ids": []
  },
  "contract": {"goal": "Исследовать следующую границу сервиса"}
}
```

Старая история остаётся нетронутой. Создаются новые conversation/session без `--resume`, сохраняются source task IDs и происхождение handoff. Mode, cwd, постоянная policy и разрешённые model/thinking не расширяются; новая цель не наследует старую приёмку. Явные execution overrides остаются явными. Чужие артефакты handoff, active/recovery claims, managed binding, source review binding, неопределённая прежняя работа и provider-policy stop запрещают этот путь. Токены, grants и verdicts не копируются. Для нерешённого исполнения используйте существующее operator/report-only восстановление; fresh не обходит отказ и не создаёт мнимое независимое ревью.

### Корректирующее ревью и ограниченные ответы

Новый `ReviewRequest` может содержать `corrective` с `previous_review_id`, его точным `previous_code_fingerprint` и `open_findings: [{"finding_id": "...", "expected_revision": N}]`. IDs относятся к этому scope и прежнему снимку; изменившаяся ревизия finding не принимается молча. Новый immutable manifest показывает добавленные/удалённые/изменённые входы и помечает режим `prior_exposed`. Новая оценка нужна даже при неизменённых выбранных байтах. Старая приёмка не переносится, авторские материалы по-прежнему раскрываются только в разрешённой стадии.

Через постраничный manifest читайте `corrective.finding_details`: исходные title/description/location, условия воспроизведения и evidence закреплены на объявленной ревизии, без авторского fix-rationale/истории. Обновлять и проверять прежний finding из другой беседы может только задача этого exact corrective snapshot с объявленной ссылкой; review-run дополнительно проверяет свой reserved stage slot. Чужие, необъявленные и устаревшие изменения запрещены.

До широкого candidate-suite проверяйте delta, открытые findings и соседние входы/replay/stage границы. Это явно согласованный порядок, не разрешение пропустить обязательный тест.

Растущие части work summary/list теперь доступны страницами, а не неограниченными массивами оставшихся IDs. Передавайте `section` и `cursor` рядом с `request`; ответ секции содержит фрагмент канонического JSON в `content` и `next_cursor`. Соедините страницы и разберите JSON. `section="blockers"` включает блокеры карточки и шагов. Изменение ревизии, reader/stage или содержимого секции делает курсор устаревшим. Явные `view=full|plan` и Markdown-report остаются полными, намеренно более крупными запросами.

Указатель страницы содержит готовые `arguments`: `request` и `section` находятся на правильном соседнем уровне. Передайте этот объект аргументами инструмента, добавляя полученный cursor для следующей страницы; не кладите параметры представления внутрь `request`.

### Не менять рабочий runtime во время проверки кандидата

Launcher уже готовит non-editable поколения. Для разработки самого Tandem сначала выполните `python -I server.py --prepare` (возвращает `key` и `python`), затем явно закрепите проверенное поколение: `--runtime-pin KEY`. `--runtime-info` показывает выбор; `--runtime-refresh` готовит и закрепляет текущий source для **будущих** запусков; `--runtime-unpin` возвращает обычное следование обновлениям. Небезопасный или устаревший pin вызывает отказ, не скрытое переключение. Уже работающие процессы/поколения не заменяются.

`python -I server.py --candidate --candidate-smoke --project-root /absolute/fixture` проверяет текущий кандидат в новом приватном state с локальной диагностикой, без запроса провайдеру. Candidate отказывается от inherited managed authority/явного live-state, отключает legacy import/channel/webhook и возвращает путь оставленного временного state. После проверки удаляйте только этот fixture-каталог. Настоящая provider-проверка — отдельное явно разрешённое действие.

Новый runtime проверяет схему БД до миграции. Это не ограничивает задним числом старый бинарник, который игнорирует guard: перед намеренным обновлением остановите/осмотрите старые клиенты и сохраните backup. Совпадение source checkout не является поводом запускать кандидат на рабочей БД пользователя.
...
В сценарии `tandem_review_run` есть отдельный **общий `budget_seconds`**: по умолчанию 600 секунд, допустимо 10–7200. Он охватывает захват и запуск, оба этапа и ожидание уточнений. Срок каждого этапа берётся из `execution`/профиля и дополнительно ограничивается оставшимся общим временем. Например, `quick` с общим бюджетом 1200 не получает 1200 секунд на первый этап: его профильный предел остаётся 600; `deep` с общим бюджетом 600 также не получает час. `wait_seconds` (0–1200) ограничивает только один вызов MCP и не увеличивает бюджет.
...
OMP по умолчанию получает закреплённую капсулу с полными обязательными инвариантами и reader для остальных данных; `context_options.delivery="full"` явно включает весь снимок. Он может предлагать изменения, но не получает инструмент публикации снимков. Отчёты ссылаются на известные правила через `rule_references`, на решения — через `decision_references`. Неизвестные IDs отклоняются; корректная ссылка не доказывает вывод. См. [контекст без повторов](#context-efficient-work).
...
Опциональный `tandem_audit` включается оператором через `OMP_TANDEM_JEV_AUDIT=1` или `--jev-audit` и `OPENROUTER_API_KEY` в окружении MCP. Он вызывается явно после чтения завершённого отчёта, сравнивает обычные acceptance-критерии (либо структурированные обязательства) с описаниями доказательств и не меняет исход задачи, проверки или приёмку. В OpenRouter/TypeSafe передаются цель, выбранные критерии, ответ/сводка и описания проверок; сами эти тексты могут содержать чувствительные сведения. Контекст задачи, команды, файлы, содержимое артефактов и история не подгружаются. Повтор той же страницы использует сохранённую попытку, включая ошибку; после неопределённого прерывания автоматического повтора нет. Другие страницы могут оплачиваться отдельно. Обычные result/wait/finish Jev не вызывают; совет нельзя передавать независимой стадии ревью. [Полный контракт, ограничения и настройка](guide.md#optional-jev-evidence-audit).

Skill `jev-audit` предлагает сначала `tandem_audit` с `preview:true`: полный допустимый payload, адрес, размер и хэш без ключа, сети и записи попытки. Отправка с `expected_input_sha256` разрешена только при обычном opt-in и ключе; изменившийся вход отклоняется до запроса. Preview не даёт согласия и не очищает прежнюю ошибку.

**Невыпущенное дополнение этой рабочей копии:** `tandem_recommend` рекомендует один скилл или направление проверки до создания задачи. В опубликованной 3.11.0 есть только аудит; сначала проверьте `tandem_scope.jev_recommend` фактически запущенного runtime. Не перезапускайте активную работу ради нового инструмента.

Передайте `request` с `kind: "skill" | "review_direction"`, `goal` до 4000 символов и 1–20 кандидатами `{id, description}`: уникальный простой ID до 64 символов, описание до 2000. Каталог задаёт координатор; файлы скиллов, контекст, история и артефакты автоматически не читаются. По умолчанию `preview:true` показывает весь точный запрос без ключа, сети или записи попытки; общий предел — 32000 UTF-8 байт, без усечения. Обычная инициализация runtime/БД может создавать служебную структуру.

После согласия на конкретный экспорт отправка требует отдельного `--jev-recommend` / `OMP_TANDEM_JEV_RECOMMEND=1`, ключа, `preview:false` и совпадающего `expected_input_sha256`. Включение аудита не включает рекомендации и наоборот. Хэш связывает цель, вид, ID/описания/порядок кандидатов, вопрос и модель, но не доказывает согласие человека. Тексты могут содержать секреты — это не редактирование конфиденциальных данных. Отправленный запрос и исход сохраняются в приватном журнале проекта; точные повторы используют прежний результат, а неизвестное исполнение не повторяется даже после перезапуска.

Ответ различает `candidate` с точным `candidate_id`, `none` и `unclear`; отказ провайдера — отдельная недоступность. ID и вероятности не доказывают наличие скилла или пригодность кода. Нет автозапуска, смены прав, отбрасывания обязательных проверок или приёмки. Совет нельзя передавать в независимую стадию ревью. Точность живого Jev не измерена синтетическими тестами. [Полный контракт и пример](guide.md#optional-jev-candidate-recommendation-unreleased).

**Теневой роутинг моделей (ещё не выпущен)** встроен в запуск новой обычной задачи. Оператор задаёт `--jev-routing-policy /absolute/policy.json` или `OMP_TANDEM_JEV_ROUTING_POLICY`: режим только `shadow`, 2–3 точных уникальных `provider/model`, описания и явно заявленные возможности. Фактическая модель не переключается. Явная модель задачи/процесса, любые продолжения, snapshot-ревью и управляемые попытки обходят роутер.

Для экспорта нужны одновременно `policy.allow_external_summary=true`, ключ и `execution.routing.allow_external_summary=true` с явно одобренным `summary` (до 4000 символов). Также задаются положительные `input_token_estimate` / `output_token_estimate`, входные `input_modalities` (по умолчанию `["text"]`) и необязательный `max_candidate_cost_usd`. Сводка не извлекается из промпта или истории. Это условия для гипотетического кандидата, а не лимиты/гарантии фактической задачи. Политика и вход закрепляются атомарно с задачей; preflight не запускает процесс, запрос или попытку.

Отдельный native-процесс без сессии, промпта, инструментов, расширений, скиллов и правил читает каталог и останавливается до Jev и основного промпта. Каталог не доказывает авторизацию или успешное исполнение. Код проверяет совпадение пула, модальности, ёмкости, thinking и заявленную поддержку инструментов; неизвестная цена не становится нулём. При менее чем двух подходящих вариантах — `no_comparison` без Jev. Иначе отправляются только одобренная сводка и разрешённые сведения о кандидатах, без путей, токенов, headers, base URL и полного контекста.

Результат виден в `execution.routing`, отдельно от реальной модели и её учёта. Бюджет работы роутера — 10 секунд по умолчанию, максимум 30 и не больше остатка задачи; патологическая остановка ОС не имеет обещанной жёсткой границы. `stop_returned` означает возврат SDK, не доказанную остановку всех потомков. Обычная ошибка оставляет исходный выбор; отмена/общий deadline запрещают последующий промпт, ошибка teardown не объявляется безопасным fallback. Неопределённые запросы не повторяются, совет не попадает в промпт или приёмку. Точность и экономия пока не измерены. [Пример политики и полный контракт](guide.md#task-start-shadow-model-routing-unreleased).
...
| `tandem_start` | `cwd`, `prompt` или `contract`, `mode`, тайм-ауты, execution/review/context, `preflight` | Проверка допуска либо явный запуск задачи |
| `tandem_continue` | `conversation_id`, `prompt` или `contract`, тайм-ауты, execution/context, `continuation`, `handoff`, `preflight` | Продолжение истории либо явно новая ограниченная беседа |
| `tandem_result` | `task_id`, `wait_seconds`, `details` | Чтение ответа, исхода, вопроса, артефактов и диагностики |
| `tandem_audit` | `task_id`, `offset=0`, `limit=5` (1–10) | Явный опциональный аудит описанных доказательств через Jev; совет, не проверка и не приёмка |
| `tandem_recommend` (ещё не выпущен) | `request`, `preview=true`, `expected_input_sha256` | Совет по явно переданным кандидатам без поиска скиллов и запуска |
...
| `tandem_context_read` | `task_id`, `pointer`, `offset_bytes`, `max_bytes` | Страницы только закреплённого product snapshot задачи; у независимого ревью свой reader |
...
Опрос — обычный и полностью функциональный способ работы для каждого поддерживаемого MCP-клиента. Для сценария используйте `tandem_review_run(action="status", run_id, wait_seconds=...)`; для отдельных задач — `tandem_result` или `tandem_wait`. Для сотрудничества с напарником Channels не требуются.
...
- **Одно длинное ожидание вместо множества коротких.** Координатор, которому нечем занять время ожидания, платит ход модели за каждое ожидание, а при вооружённом watchdog обычное ожидание отпускается сразу — повторять его значит крутиться вхолостую. Просите то ожидание, которое вам действительно нужно: `wait_seconds` до **1200** с `wait_mode="bounded"`, то есть ожидание наблюдаемого состояния, а не появления доставки. Сервер отрабатывает ожидание целиком; сколько клиент готов просидеть в переднем плане — отдельный вопрос, на который сервер не отвечает. Клиенты ведут себя по-разному, поэтому ветвитесь по тому, что написано в ответе, а не по числу.
- **Граница клиента: 120 секунд у Claude Code.** В одном измеренном прогоне каждый запрос, ещё не завершившийся к **120 секундам**, переводился в фоновую задачу: немедленный ответ прямо сообщал об этом и называл задачу, ожидание продолжало идти, а настоящий результат приходил позже уведомлением — доставлены все десять таких вызовов, медиана 235 секунд после передачи. Это наблюдение за одним клиентом в один день, а не гарантия и не порог, под который стоит проектировать. Если ответ сообщает о такой передаче, считайте это доставкой, а не сбоем: сохраните идентификатор, прочитайте тот результат и не запускайте второе наблюдение по привычке. Остановка ожидателя на стороне клиента не отменяет работу, но фоновая задача не переживает выхода из сессии. Если клиент действительно отменил вызов или упал по таймауту, вернитесь к ограниченному чтению той же задачи или того же прогона — не к новому запуску.
- **Опрос (`delivery=poll`):** задачи не будят координатора. Делайте взаимодополняющую работу либо ожидайте через `tandem_result(wait_seconds=...)` для одной задачи или `tandem_wait(task_ids, wait_seconds=...)` для нескольких, затем читайте готовые результаты. Обрабатывайте вопросы и исключайте обработанные терминальные ID. Повторяйте, пока есть активная работа; без циклов с нулевым ожиданием, опроса `tandem_list` и обещаний будущего уведомления. Настройка каналов и управление вебхуками не входят в процедуру принудительного polling.
- **Подтверждённый push (`delivery=push`):** событие ускоряет получение результата, но само по себе не заменяет ограниченный polling. `await_event` допустим лишь при живом вооружённом watchdog для выбранных задач и подтверждённом независимом пробуждении. Тогда оставьте клиент открытым, делайте другую работу и по событию или сигналу watchdog прочитайте результат. При отсутствии такого watchdog вернитесь к тому же ограниченному ожиданию.
...
- **Откат:** сбой транспорта, истечение срока или потеря watchdog возвращают ограниченное ожидание. Не используйте цикл с `wait_seconds=0`. Ожидание остаётся конечным и отменяемым, оно никогда не продлевает бюджет хода, срок вопроса или бюджет ревью, а его истечение сообщается в `wait`, а не как состояние задачи. Подтверждение вебхука через `event_id` и чтение результата не дают права повторять эффекты: для этого действует отдельный протокол квитанций.
...
| Одно ожидание MCP | До 1200 секунд на стороне сервера; что клиент делает с ещё не завершившимся запросом — его собственное поведение (в одном измеренном прогоне Claude Code переводил такой запрос в фоновую задачу на 120 секундах). У `tandem_diagnose` предел остаётся 25 секунд |
