...
**Версия: 3.3.0** · [Лицензия MIT](../LICENSE) · [Релизы](https://github.com/Flyozzzz/omp-tandem-public/releases) · [Channels и вебхуки (на английском)](channels.md) · [Oh My Pi](https://github.com/can1357/oh-my-pi)
...
Доступны **19 MCP-инструментов**. Для первого ревью начните со сценария `tandem_review_run`; отдельные инструменты нужны для более точного ручного управления. Префиксы зависят от клиента; ниже приведены стабильные суффиксы. MCP `Context` внедряется автоматически и не является пользовательским аргументом.
