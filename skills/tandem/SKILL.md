...
Continuation for superseded attempts is `checkpoint` when validated bytes fit new ownership, `blocked` if the step was removed or ownership shrank, or `not_available` without validated bytes (including capture failure). These are continuation outcomes, not automatic card blockers. Inspect failure and remaining work; never treat output/report/intents as accepted bytes or rerun prior effects by default. A checkpoint is returned to the new claim with commit/base/operator evidence, not transferred acceptance. Before begin, withdrawal only drops the proposal; after begin it leaves sticky quiescence: fenced attempts still need operator reconcile and any pause needs explicit resume. Emergency reconcile remains the operator fallback, never an agent escape hatch; retry does not replay, abandon leaves a blocker/pause.
...
Require fresh child-scope agreements and independent review/acceptance of the exact child submission. `show WORK --format markdown` displays closure/continuation/predecessors. Acceptance is not application. Local 3.7.0 wheel/ZIP packaging and checks are a proposal, not a tag, publication, plugin install or application; only the operator decides those actions. See `docs/guide.md#plan-transitions` and `#repository-handover` for the documented CLI/MCP scenarios. They do not establish a live-provider replan scenario; preserve actual evidence boundaries.
