...
The unreleased candidate-recommendation tool has a separate `--jev-recommend` / `OMP_TANDEM_JEV_RECOMMEND` opt-in; audit enablement does not enable it. Inspect the loaded runtime's `jev_recommend` capability, not the checkout alone. Preview works without a key; sending additionally needs explicit approval and the exact preview hash. Never fetch credentials, self-enable or restart active work to make advice available.

Unreleased model routing is independently configured by `--jev-routing-policy PATH` / `OMP_TANDEM_JEV_ROUTING_POLICY`. It is shadow-only, not an active model selector. Require an operator-chosen 2–3-model pool, honest capability assertions and separate policy/per-task summary export approval; never invent model identities or remove an explicit model override to make routing eligible. Inspect `tandem_scope.model_routing`; missing Jev access is a bypass. A metadata-only native probe adds startup work but no inference prompt. Do not claim catalog membership proves authentication or that SDK `stop_returned` proves all descendants exited. See `docs/guide.md#task-start-shadow-model-routing-unreleased`.
