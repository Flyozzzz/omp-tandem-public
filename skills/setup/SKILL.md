...
Continuation records `checkpoint` for validated bytes within new ownership, `blocked` for removed steps or shrunken ownership, `not_available` when validated bytes are absent, including capture failure. These outcomes are not automatic card blockers; inspect failures and remaining work before proceeding. The checkpoint carries commit/base/operator evidence into the new claim, not transferred acceptance or a replay of old commands. Before begin, withdraw only removes the proposal; after begin it leaves sticky quiescence: fenced attempts need operator reconcile and any pause needs explicit resume. Emergency reconcile remains the operator fallback; retry never replays automatically and abandon leaves a blocker/pause.
...
Local 3.7.0 packaging (`uv build --wheel`, `uv run --frozen python scripts/package.py`, then `--check`) is a proposal, not publication, tagging, installation into user sessions or application; only the operator decides those actions. See `docs/guide.md#plan-transitions` and `#repository-handover` for CLI/MCP scenarios and honest evidence limits; no live-provider replan is established by a passing localhost compatibility fixture.
