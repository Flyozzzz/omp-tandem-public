...
Before activating an operator grant, explain `authorize --preview` (`stored=false`, no authority/readiness proof), model flags before authorize, and `--max-attempt-cost-usd`/`--allow-shell` after it. `preview.model_selection` discloses Claude `fixed_selector` (default `sonnet`) and OMP `fixed_selector` with explicit `--omp-model`/`--model`, or `dynamic_default` without it: OMP resolves its configured selection at each attempt start. Recommend `--omp-model` for predictable autonomous runs; selectors are not observed identity. `legacy_unpinned` needs reauthorization and `malformed` is labelled on reads/refused at launch. Run/start cannot override policy. Markdown `## Authorization` shows the persisted authorized_at/deadline window, permissions and per-seat policy. The attempt ceiling defaults to half the total independently of max_launches; preview permissions/reserves show capabilities. `--allow-tests` is the deprecated arbitrary-shell alias, not a test sandbox. Shell-granted review creates an operator blocker before launch; never silently waive checks. Managed reviewers use pinned snapshots, independent report, optional comparison, then verdict. Helpers remain disabled; see `docs/helper-compatibility.md`.
...
`tandem_work` defaults to `view="summary"`; sibling `view=plan|step|full`, `format`, `limit`, `cursor`, `include_snapshots` select permitted material. History is paged without snapshots by default; follow continuations and refetch after `cursor_stale`. `next_actions` are hints. `wake_acknowledgment` distinguishes `acknowledged`, `acknowledged_zero`, `deferred` and `not_attempted`; a deferred ack is not a failed state read or permission to relaunch.
...
Local 3.7.1 packaging (`uv build --wheel`, `uv run --frozen python scripts/package.py`, then `--check`) is a proposal, not publication, tagging, user-session installation or application; only the operator decides. See `docs/guide.md#plan-transitions` and `#repository-handover`. The deterministic managed-replan fixture proves real supervised Claude-child stop and checkpoint continuation with one fixture effect; not generic exactly-once effects, OMP-native teardown or live-provider replan.
