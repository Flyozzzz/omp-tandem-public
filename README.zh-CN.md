...
当前软件包版本为 **3.8.1**。[上下文胶囊、启动前检查、新会话与修正复审](docs/guide.zh-CN.md#context-efficient-work) 减少重复协调，但不移除权限或验收检查。详见[变更日志](CHANGELOG.md)。构建／检查软件包不会创建标签、发布版本、安装到用户会话或应用代码。
